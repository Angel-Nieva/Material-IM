...
# 4. Haciendo un poco de investigaci?n sobre el paquete caret, en particular c?mo hacer 
# Recursive Feature Elimination (RFE), construir un modelo de regresi?n lineal m?ltiple 
# para predecir la variable IMC que incluya entre 10 y 20 predictores, seleccionando el 
# conjunto de variables que maximice R2 y que use cinco repeticiones de validaci?n 
# cruzada de cinco pliegues para evitar el sobreajuste (obviamente no se debe considerar 
# las variables Peso,  Estatura ni estado nutricional –Weight, Height, EN respectivamente).

# Control de RFE usando random forest con validaci?n cruzada de 5 pliegues
# con 5 repeticiones
...
# n?meros de posiciones de el vector calculados anteriormente.
...
# Usando RFE, construir un modelo de regresi?n log?stica m?ltiple para la variable EN que incluya el conjunto, de entre dos y seis, predictores que entregue la mejor curva ROC y que utilice validaci?n cruzada dejando uno fuera para evitar el sobreajuste (obviamente no se debe considerar las variables Peso, Estatura ?Weight y Height respectivamente? ni IMC).
...
umbral <- 0.5
# Aplicar a cada probabilidad predicha la funci?n que determina si pasa
# el umbral o no.
preds_e <- sapply(probs_e, function(p) ifelse(p >= umbral, "1", "0"))
preds_e <- factor(preds_e, levels = levels(entrenamiento_logistico[["EN"]]))

# Evaluaci?n del clasificador: Gr?fico ROC y matriz de confusi?n.


# Es mejor mientras la curva se aleje m?s de la diagonal.

# Matriz de confusi?n (VP,FP,VN,FN)
...
# 1. El modelo tiene predictores cuantitativos y dicot?micos.
...
# una significaci?n de 0.05, aunque est?n al borde.
...
# 5. Verificando la condici?n de independencia de los residuos con Dubrin-Watson.
...
# El vif m?ximo es de 7, se podr?a considerar observaci?n preocupante los que
...
# 7.  Los valores de la variable de respuesta son independientes entre s? y
# viendo los datos los predictores no son constantes, adem?s cada predictor
...
# diferencia, aunque est? algo ajustado.
...
# El vif m?ximo es de 6, se podr?a considerar observaci?n preocupante pero no
# supera 10 y ninguno
# m?s tiene valores preocupantes.
...
# Aqu? el MSE de prueba es a?n mayor que el de entrenamiento y, comparado con el modelo
# anterior,y ambos mucho mayores, por lo que es menos generalizable que ?l y est? 
# sobre ajustado. 

# Evaluando el modelo de regresi?n log?stica:
...
# aunque est? muy escalonado, por lo que le faltar?a un poco m?s de precisi?n.

# Viendo bondad de ajuste con log-verosimilitud.
print(logLik(modelo_logistico))

# lnL = -18.56, al ser negativo se podr?a considerar bajo y por lo tanto
